...
function RouteComponent() {
	...
	const [_assetSearch, _setAssetSearch] = React.useState('')
					...
					<div className="relative">
						...
						>
							...
							<input
								...
								className="bg-transparent outline-none text-[14px] text-primary placeholder:text-secondary w-[80px] sm:w-[100px] focus:w-[140px] sm:focus:w-[180px] transition-all"
							/>
						</form>
						{searchFocused &&
								...
								<div className="absolute top-full left-0 right-0 mt-1 bg-surface border border-card-border rounded-full shadow-xl overflow-hidden z-50">
									...
									>
										<div className="flex flex-col min-w-0">
											<span className="text-[12px] text-primary font-mono truncate">
				...
				<div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-4 mb-5">
					<div className="flex-1 min-w-0 flex flex-col gap-2 order-2 sm:order-1">
						...
						<div className="flex items-center gap-1.5 max-w-full">
							...
							>
								<code className="text-[12px] sm:text-[14px] font-mono text-secondary leading-tight min-w-0 text-left group-active:translate-y-px">
									{address.slice(0, 21)}
									<br />
									{address.slice(21)}
								</code>
										...
										extra: (
											<>
												...
												<button
													...
													className="flex items-center gap-1.5 text-[13px] text-tertiary hover:text-primary font-mono transition-colors cursor-pointer"
												>
													<span>
...
function ActivityHeatmap({
	...
	// Show last ~7 days of activity spread across the grid
	...
	// Blocks per slot - aim for ~7 days coverage
	// ~2 blocks/sec × 60 sec × 60 min × 24 hours × 7 days = 1,209,600 blocks/week
	const blocksPerWeek = 1209600n
	const blocksPerSlot = blocksPerWeek / BigInt(totalSlots)

	const activityBySlot = React.useMemo(() => {
		const counts = new Map<number, number>()
		if (!currentBlock) return counts

		const oldestBlock = currentBlock - blocksPerWeek
	...
	const formatSlotTime = (slotIndex: number) => {
		...
		const hoursAgo = ((totalSlots - 1 - slotIndex) / totalSlots) * 24 * 7
	...
	return (
		<div className="relative">
			...
			{hoveredCell &&
				createPortal(
					<div
						className="fixed z-[100] px-2 py-1 text-[12px] text-white bg-gray-900 rounded-md shadow-lg whitespace-nowrap pointer-events-none border border-gray-700"
						style={{
							left: hoveredCell.x,
							top: hoveredCell.y - 6,
			...
			<div className="flex flex-col gap-1.5 mt-2 mb-3">
				...
				<div className="flex items-center justify-center">
					<div className="flex items-center gap-1 h-5 px-2 rounded-full bg-white/5 border border-white/10">
						...
						<span className="text-[12px] text-tertiary font-mono">...</span>
					</div>
				</div>
			</div>
		)
		...
		>
			...
			<div className="flex items-center justify-center">
				...
				>
					...
					<span className="text-[12px] text-tertiary">{t('common.block')}</span>
					<span className="text-[12px] text-primary font-mono tabular-nums">
						{selectedBlock !== undefined
...
function HoldingsTable({
	...
	return (
		<>
			<div className="text-[14px] -mx-2 flex flex-col">
	...
	return (
		<>
			<button
				...
				className={cx(
					'h-[34px] px-2 rounded-lg border bg-white/5 text-[13px] flex items-center gap-1.5 transition-colors cursor-pointer',
					isOpen
						? 'border-accent'
				...
				createPortal(
					...
					>
						<button
							...
							className={cx(
								'w-full px-2 py-1.5 text-[13px] text-left hover:bg-base-alt transition-colors cursor-pointer flex items-center gap-1.5',
								!selectedKey ? 'text-accent' : 'text-primary',
							)}
						>
						...
						{accessKeys.map((key) => {
							...
							return (
								<button
									...
									className={cx(
										'w-full px-2 py-1.5 text-[13px] text-left transition-colors flex items-center gap-1.5',
										!isAvailable
...
function AssetRow({
	...
	// Also add a timeout to recover if the balance never updates
	React.useEffect(() => {
	...
	if (isExpanded) {
		return (
			...
			>
				...
				<div className="flex items-center gap-1.5 px-1">
					<div className="flex items-center flex-1 h-[34px] rounded-lg border border-card-border bg-white/5 focus-within:border-accent">
						<input
							...
							placeholder="0x..."
							className="flex-1 min-w-0 h-full px-2 bg-transparent text-[14px] text-primary font-mono placeholder:text-tertiary focus:outline-none"
						/>
					</div>
				</div>
				...
				<div className="flex items-center gap-1.5 px-1">
					...
					<div className="flex items-center flex-1 h-[34px] rounded-lg border border-card-border bg-white/5 focus-within:border-accent">
						...
						<input
							...
							placeholder="0.00"
							className="flex-1 min-w-[40px] h-full px-1.5 bg-transparent text-[16px] text-primary font-mono placeholder:text-tertiary focus:outline-none"
						/>
						<span className="text-[12px] text-tertiary font-medium shrink-0 pr-1">
							{asset.metadata?.symbol || '???'}
						</span>
						<button
							type="button"
							onClick={handleMax}
							className="h-full px-2 border-l border-card-border text-[11px] font-medium text-accent hover:bg-base-alt cursor-pointer transition-colors shrink-0"
						>
							MAX
						</button>
					</div>
				...
				{sendError && (
					<div className="pl-[30px] text-[11px] text-negative truncate">
						{sendError}
					</div>
				)}
			</form>
		...
		>
			<span className="pl-1 pr-2 text-primary flex items-center gap-1.5">
				...
				<span className="flex flex-col min-w-0">
					...
					<span className="text-[12px] text-tertiary font-mono truncate">
						{asset.metadata?.symbol || shortenAddress(asset.address, 3)}
					</span>
				</span>
			</span>
			...
			>
				...
				>
					<span className="text-primary font-sans text-[15px] tabular-nums text-right truncate max-w-full">
						{asset.balance !== undefined &&
					...
					<span className="text-secondary text-[12px] md:hidden whitespace-nowrap">
	...
	const tabButtons = (
		<div className="flex items-center gap-3">
			<button
				...
				className={cx(
					'text-[13px] font-medium pb-0.5 border-b-2 cursor-pointer',
					activeTab === 'mine'
				...
				<div className="flex items-center justify-center gap-1 pt-3 pb-1">
					{Array.from({ length: totalPages }, (_, i) => (
						<button
							...
							onClick={() => setPage(i)}
							className={cx(
								'size-[28px] rounded-full text-[13px] cursor-pointer transition-all',
								page === i
		...
		<>
			...
			>
				...
				<div className="flex items-center gap-2 shrink-0">
					{item.timestamp && (
						<span className="text-[12px] text-tertiary font-mono tabular-nums">
			...
			>
				...
				>
					<div className="flex flex-col sm:flex-row gap-4 sm:gap-[40px] px-4 sm:px-[20px] pt-5 sm:pt-[24px] pb-4 sm:pb-[16px]">
						...
						<div className="flex flex-col gap-[8px] font-mono text-[13px] sm:text-[14px] leading-[16px] flex-1">
					...
					{filteredEvents.length > 0 && (
						<>
							...
							<div className="flex flex-col gap-3 px-4 sm:px-[20px] py-4 sm:py-[16px] font-mono text-[13px] sm:text-[14px] leading-4 [counter-reset:event]">
				...
				<div className="w-full max-w-[360px]">
					<a
						...
						target="_blank"
						rel="noopener noreferrer"
						className="press-down text-[14px] font-sans px-[12px] py-[12px] flex items-center justify-center gap-[8px] bg-white/5 hover:bg-white/10 rounded-bl-[16px] rounded-br-[16px] text-tertiary hover:text-primary transition-colors"
					>
